...
from model.models import EcgResNet34  # or import ZolotyhNet if desired
...
def train_local_model(model, train_loader, device, epochs=1, learning_rate=0.001):
...
def add_noise(model, sensitivity, epsilon):
    ...
    with torch.no_grad():
        for param in model.parameters():
            # *Changed*: Lowered default noise to reduce accuracy drop
...
if __name__ == '__main__':
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        try:
            ...
            # *Changed*: Slightly reduced sensitivity for better accuracy 
